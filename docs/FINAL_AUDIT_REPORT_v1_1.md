# Migrationsaudit Dinas → AX — Abschlussbericht v1.1
...
| Kunden-Scopes (KNR-Einheiten) | 12 |
| Pool gesamt (ef > 0) | 19.965 Abrechnungszeilen |
| Davon beurteilbar | 17.496 Zeilen (87,6 %) |
| Erlösvolumen beurteilbar | **7,61 MEUR** |
| Auditierter Zeitraum | Oktober 2025 – März 2026 |

### Schlüsselbefund

**Die Migration Dinas → AX wurde sauber durchgeführt.**  
Über 12 Kunden-Scopes und ein Erlösvolumen von 7,61 MEUR wurde **kein systematischer Migrationsschaden** identifiziert.

| Kennzahl | Wert |
|---|---|
| Net Δ gesamt (nominal) | +98.005 EUR (+1,29 %) |
| Net Δ gesamt (adjustiert) | **+146.867 EUR (+1,93 %)** |
...
Ein Sika-Teilscope wird in einer Folgesession abgeschlossen (keine Änderung der Gesamtaussage erwartet):
- KNR 527406 ATM Schweiz (separate BI-Quelle)

KNR 511241 Import-Flow wurde in v1.1 abgeschlossen: 346 Rows, 629.422 EUR ef, Net Δ P20-adj +9.915 EUR (+1,55 %) — kein Migrationsschaden.

### Vertrauen in die Aussage

12 Kunden-Scopes zeigen konsistent: kein Kunde weist einen Befund auf, der auf systematische Tarif-Übertragungsfehler hindeutet. Calculator-Pre-Flight-Fixes (P1–P20) wurden bei 6 Kunden vor der Analyse durchgeführt und sind nicht auf Migrations-Versagen zurückzuführen.
---
...
| Dimension | Wert |
|---|---|
| Kunden | 9 Unternehmen (12 KNR-Scopes) |
...
**Erlös-Komponenten-Separation (P20):** AX bucht manche Erlös-Komponenten in separaten Posten. Drei Sub-Pitfalls: P20.1 Maut — wenn DLV integriert und AX separiert, wird `ef_total = ef + Erlöse_Maut` verwendet (betrifft Sika 491063 +30.555 EUR, Sika Import +18.306 EUR). P20.2 Diesel — alle Calculator liefern `diesel_surcharge=None`, kein systematischer M2-Bias durch Dieseltrennung (retroaktiver Check für alle 12 Scopes: 0 Adjustierungen). P20.3 weitere Komponenten — Checkliste für Folgekunden.
...
| Sika Import | 511241 | 346 | 346 | 299 | 27 | 20 | 629.422 | −8.392 | −1,32 % | kein Schaden⁷ |
| **Gesamt** | | **19.965** | **17.496** | **14.700** | **1.259** | **1.537** | **7.608.441** | **+98.005** | **+1,29 %** | **0 Schäden** |

**Adjustiertes Gesamt** (nach P20-Korrekturen Sika 491063 + Import-Flow): Net Δ = **+146.867 EUR (+1,93 %)**
...
**⁷ Sika Import Dieselfloater-Artefakt:** Nominelles Net Δ −8.392 EUR (−1,32 %) ist durch den IT-Dieselfloater bedingt. AX schlägt einen quartalsweisen Dieselfloater auf die LKW-Flatrate auf; das DLV enthält nur die Basisrate. Σ Erlöse Diesel BI (IT-28065) = 15.773 EUR. P20.1-adjustiert (ef + Erlöse Maut 18.306 EUR): Net Δ = **+9.915 EUR (+1,55 %)**. ES-28108 (113 Rows): 100 % M1 P20-adj, Net Δ = −557 EUR (−0,21 %). Kein Migrationsschaden.

### M-Klassen-Verteilung Gesamt

| Klasse | Rows | Anteil | Δ-Beitrag (EUR) |
|---|---:|---:|---:|
| M1 (±5 %) | 14.700 | 84,0 % | +53.366 |
| M2 (< −5 %) | 1.259 | 7,2 % | −158.819 |
| M_over (> +5 %) | 1.537 | 8,8 % | +203.459 |
| **Gesamt beurteilbar** | **17.496** | **100 %** | **+98.005** |
---
...
**Calculator:** `sika_import.py` — `SikaImportCalculator` (Commit 4d38156) | **DLV:** Import Spanien und Italien 2025/2026  
...
**Scope:** Physische Import-Sendungen von den Sika-Produktionsstandorten Alcobendas/ES (PLZ 28108) und Cerano/IT (PLZ 28065) zum Sika-Lager Stuttgart DE-70499. Zeitraum: Sep 2025 – Mär 2026. Abgrenzung: de_oos-Rows mit Versender-PLZ ∈ {28108, 28065}.

**Pricing-Modi:**
- **ES-28108 (Alcobendas, 113 Rows):** per-Stellplatz-Tarif (1–34 Paletten, EUR/Sendung). 113/113 M1 nominal + P20-adj. Net Δ P20 = −557 EUR (−0,21 %).
- **IT-28065 (Cerano, 233 Rows):** Komplett-LKW Flat-Rate (LKW 1 ≤ 33 Stpl. = 1.495/1.562 EUR; LKW 2 > 33 Stpl. = 1.575/1.645 EUR). Net Δ P20 = +10.472 EUR (+2,84 %).

**Hauptbefund — IT Dieselfloater-Artefakt:**  
IT-Rows zeigen fp-Streuung von −12 % bis +38 % um den DLV-Soll. Das DLV enthält ausschließlich die Basisrate; der quartalsweise Dieselfloater (ab 01.03.2024) wird in AX als Faktor aufgeschlagen. Σ Erlöse Diesel BI (IT-28065) = 15.773 EUR (≈ 4,3 % des IT-ef). Aggregat IT Net Δ P20 = +10.472 EUR (+2,84 %) — kein systematischer Migrationsschaden.

**P20.1-Adjustierung:** `ef_total = Erlöse Fracht + Erlöse Maut`. Σ Erlöse Maut = 18.306 EUR. DLV-Soll = `basispreis + maut_surcharge`.

**Calculator Pre-Flight:** P4 (0 Sub-Rows), P6 (0 stp=0), P14 (OOS-Ausschluss via PLZ-Filter), P16 (Year-Dispatch 182/164 Rows), P20.1 (Erlöse Maut 18.306 EUR) — alle clear. DLV-Lücke: 0.
...
| P20.2 | Erlöse-Diesel-Separation (Calculator diesel=None, kein M2-Bias) | IT Dieselfloater als M_over/M2-Streuung dokumentiert; 0 Adjustierungen | 0 |
...
**Status:** **Auditiert.** 346 Rows (629.422 EUR ef), 0 DLV-Lücken. Net Δ P20-adj = **+9.915 EUR (+1,55 %)** — kein Migrationsschaden.

**Scope:** de_oos-Rows mit Versender-PLZ ∈ {28108 (ES-Alcobendas), 28065 (IT-Cerano)} → DE-70499 Stuttgart. Calculator: `SikaImportCalculator` (Commit 4d38156). Pipeline: `build_sika_import_step23_v197.py` (Commit 07ba458).

**Befund:** ES-28108 vollständig M1 (113/113). IT-28065 zeigt fp-Streuung durch quartalsweisen Dieselfloater (Σ Erlöse Diesel = 15.773 EUR). Aggregat IT Net Δ P20 = +10.472 EUR (+2,84 %) — innerhalb Wesentlichkeitsschwelle.
...
|---|---|---|
| ~~KNR 511241 Import-Flow~~ | ~~1 h~~ | **Abgeschlossen (v1.1)** |
...
>
> Über 12 Kunden-Scopes (9 Kundenfirmen), 17.496 beurteilbare Abrechnungszeilen und ein Erlösvolumen von 7,61 MEUR wurde **kein systematischer Migrationsschaden** identifiziert. Kein Kunde weist eine Konstellation auf, die auf fehlerhafte Tarif-Migration hindeutet.
>
> Alle identifizierten Abweichungen sind durch methodisch erklärbare Faktoren begründet: Calculator-Pre-Flight-Fixes (P1–P20 bei 6 Kunden), Erlös-Komponenten-Separation (P20.1 Maut: Sika 491063 und Sika Import), Pricing-Mode-Artefakte (Charter-Pool Fischerwerke, GC-Band Groz-Beckert, IT-Dieselfloater Sika Import) oder FTL-Flat-Rate-Effekte (P19, Hornschuch). Diese Faktoren sind keine Migrationsfehler.

**Net Δ adjustiert (alle 12 Kunden-Scopes): +146.867 EUR (+1,93 %)**  
...
| **v1.1 (Report)** | **2026-04-28** | **12. Scope: Sika Import-Flow abgeschlossen (346 Rows, +9.915 EUR P20-adj); P20.1/P20.2/P20.3; Diesel-Check 12 Scopes** |
