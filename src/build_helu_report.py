...
Datenquellen:
  - output/helu_dinas_vergleich.xlsx   (PRE/POST already matched)
  - cluster_vergleich/408244_HELU_KABEL_GMBH_cluster.xlsx (Soll EUR per Auftrag)
"""
...
for df in (pre, post):
    ...
    df['Empfaenger PLZ']  = df['Empfänger PLZ'].astype(str).str.strip()
    df['Empfaenger Land'] = df['Empfänger Land'].astype(str).str.strip()
    df['_gwb']            = df['Tonnage (eff.)'].apply(gew_band)
    df['_plz2']           = df['Empfaenger PLZ'].apply(plz2)
    df['_cluster']        = df['Empfaenger Land'] + '|' + df['_plz2'] + '|' + df['_gwb']
...
def cluster_stats(pre, post):
    pa = pre.groupby('_cluster').agg(
        n_pre=('Dinas Gesamt','count'), avg_d_ges=('Dinas Gesamt','mean'),
        avg_d_fr=('Dinas Fracht','mean'), avg_d_di=('Dinas Diesel','mean'),
        avg_d_maut=('Dinas Maut/SSD','mean')).reset_index()
    oa = post.groupby('_cluster').agg(
        n_post=('AX Gesamt','count'), avg_ax_ges=('AX Gesamt','mean'),
        avg_ax_fr=('AX Fracht','mean'), avg_ax_di=('AX Diesel','mean'),
        avg_ax_maut=('AX Maut','mean'), avg_ax_neben=('AX Nebengebühr','mean')).reset_index()
...
print(f'Unterfakturierungs-Cluster: {len(stats)}, '
      f'Sigma Verlust: {stats["est_loss"].sum():,.0f} EUR')
...
COLS = [
    ...
    ('Empfaenger',  'Empfänger Name',              'Empfänger Name',    22),
    ('PLZ',         'Empfänger PLZ',               'Empfänger PLZ',      7),
    ('Land',        'Empfänger Land',              'Empfänger Land',     5),
    ('Ton. kg',     'Tonnage (eff.)',               'Tonnage (eff.)',      9),
    ('Soll EUR',    'Soll EUR',                    'Soll EUR',           9),
    ('Fracht EUR',  'Dinas Fracht',                'AX Fracht',         10),
    ('Diesel EUR',  'Dinas Diesel',                'AX Diesel',          9),
    ('Maut EUR',    'Dinas Maut/SSD',              'AX Maut',            9),
    ('Ausfuhr EUR', 'Dinas Ausfuhr',               'AX Nebengebühr',     9),
    ('Verzoll EUR', 'Dinas Verzollung',            'AX Lademittel',      9),
    ('ZollD EUR',   'Dinas Zoll Duty',             'AX Peak',            8),
    ('Sulphur EUR', 'Dinas Sulphur',               'AX EUST/Zoll',       9),
    ('NKP EUR',     'Dinas Nebenkostenpausch.',    'AX Versicherung',    9),
    ('Redebit EUR', 'Dinas Redebit',               None,                 8),
    ('Sonst EUR',   'Dinas Sonstige',              None,                 8),
    ('GESAMT EUR',  'Dinas Gesamt',                'AX Gesamt',         11),
]
N = len(COLS)
PRE_OVR  = {9:'D-Fracht EUR',10:'D-Diesel EUR',11:'D-Maut EUR',12:'D-Ausfuhr EUR',
            13:'D-Verzoll EUR',14:'D-ZollD EUR',15:'D-Sulphur EUR',16:'D-NKP EUR',
            17:'D-Redebit EUR',18:'D-Sonstige EUR',19:'D-GESAMT EUR'}
POST_OVR = {9:'AX Fracht EUR',10:'AX Diesel EUR',11:'AX Maut EUR',
            12:'AX Neben EUR',13:'AX Lademl EUR',14:'AX Peak EUR',
            15:'AX EUST EUR',16:'AX Versich EUR',17:'',18:'',19:'AX GESAMT EUR'}

def write_hdrs(ws, row, ovr, hf):
    for ci,(h,_,_,_) in enumerate(COLS, 1):
        wc(ws, row, ci, ovr.get(ci,h), hf, font(bold=True,color='FFFFFF',size=8),'center',brd=BRD_H)
...
def ws_zusammenfassung(ws):
    ...
    rows = [
        ('HELU KABEL GmbH (408244) -- Dinas PDF vs AX Vergleich', ''),
        ('', ''),
        ('-- PRE (Dinas-System) --', ''),
        ('BI-Zeilen PRE (gematcht)', len(pre)),
        ('Sigma Dinas Fracht EUR', round(pre['Dinas Fracht'].sum(), 2)),
        ('Sigma Dinas Gesamt EUR', round(pre['Dinas Gesamt'].sum(), 2)),
        ('', ''),
        ('-- POST (AX-System) --', ''),
        ('BI-Zeilen POST', len(post)),
        ('Sigma AX Fracht EUR', round(post['AX Fracht'].sum(), 2)),
        ('Sigma AX Gesamt EUR', round(post['AX Gesamt'].sum(), 2)),
        ('', ''),
        ('-- Cluster-Vergleich --', ''),
        ('Unterfakturierungs-Cluster', len(stats)),
        ('Sigma geschaetzter Gesamtverlust EUR', round(stats['est_loss'].sum(), 0)),
    ]
    for i,(k,v) in enumerate(rows, 1):
        ws.cell(i,1).value = k; ws.cell(i,2).value = v
        if v=='' and k.startswith('--'):
            ws.cell(i,1).fill = fill('D6E4F7'); ws.cell(i,1).font = font(bold=True)
        elif i==1: ws.cell(i,1).font = font(bold=True, size=12)
        else: ws.cell(i,2).alignment = Alignment(horizontal='right')

# -- Sheet 4: Cluster-Vergleich --------------------------------------------
def ws_cluster(ws, top_n=20):
    ws.title = 'Cluster-Vergleich'
    mrow(ws,1,1,N,'HELU KABEL GmbH (408244) -- Cluster-Vergleich: Unterfakturierung',
         fill('1F497D'), font(bold=True,color='FFFFFF',size=12),'center')
    mrow(ws,2,1,N,'PRE: Dinas NK  |  POST: AX NK  |  Soll EUR aus Tarifmotor  |  Gruen = Kontroll-Paar',
    ...
    for _,cl in stats.head(top_n).iterrows():
        ...
        row += 1
        mrow(ws,row,1,N,
             f'  Dinas Avg: {cl.avg_d_ges:,.2f} EUR  |  AX Avg: {cl.avg_ax_ges:,.2f} EUR  '
             f'|  Delta: {cl.delta:,.2f} EUR ({cl.delta_pct:+.1f}%)  '
             f'|  Gesch. Verlust: {cl.est_loss:,.0f} EUR  |  {cl.abw_grund}',
        ...
        for _,pr in pre_s.iterrows(): row += 1; write_drow(ws, row, pr, 1, fill('BDD7EE'))
        # POST
        row += 1
        mrow(ws,row,1,N,'  POST -- AX Rechnungen',fill('C55A11'),font(bold=True,color='FFFFFF'))
        row += 1; write_hdrs(ws, row, POST_OVR, fill('C55A11'))
        for _,po in post_s.iterrows(): row += 1; write_drow(ws, row, po, 2, fill('FCE4D6'))
        # Kontroll
        if kpi is not None or koi is not None:
            ...
            if kpi is not None:
                row += 1; wc(ws,row,1,'PRE',fill('E2EFDA'),font(bold=True,italic=True))
                pr = pre.loc[kpi]
                for ci,(h,ps,_,_) in enumerate(COLS[1:],2):
                    v=pr.get(ps) if ps else None
                    if v is not None and pd.isna(v): v=None
                    e='EUR' in h
                    wc(ws,row,ci,v,fill('E2EFDA'),font(size=9),'right' if e else 'left',EUR_FMT if e else None,BRD)
            if koi is not None:
                row += 1; wc(ws,row,1,'POST',fill('E2EFDA'),font(bold=True,italic=True))
                po = post.loc[koi]
                for ci,(h,_,qs,_) in enumerate(COLS[1:],2):
                    ...
                    wc(ws,row,ci,v,fill('E2EFDA'),font(size=9),'right' if e else 'left',EUR_FMT if e else None,BRD)
        row += 1
    for ci,(_,_,_,w) in enumerate(COLS,1): ws.column_dimensions[get_column_letter(ci)].width = w
    ws.freeze_panes = 'A3'; return row
