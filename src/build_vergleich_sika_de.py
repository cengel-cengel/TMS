"""Sika DE (491063) Dinas-AX Vergleich Excel."""
import sys, pickle
...
OUT = Path("output/erka_lieferung/Sika_DE_Vergleich.xlsx")

from tms.tariff.calculators.sika_de import SikaDeCalculator
_calc = SikaDeCalculator()
...
with open("output/bi_top20_data.pkl","rb") as f:
    bi = pickle.load(f)["df"]
dinas = pickle.load(open("output/dinas_cache_491063.pkl","rb"))
post  = bi[(bi["Kunden Nr BK"]==491063)&(bi["periode"]=="POST")].copy()
zgi   = load_zgi_map("Sika.xlsx")

n_clust, n_d, n_a = build_and_save(
    [491063], dinas, post, zgi, soll_fn,
    "Sika Deutschland GmbH", OUT, "Sika.xlsx")
print(f"Sika DE: {n_clust} Cluster, {n_d} Dinas, {n_a} AX  → {OUT}")
