#!/usr/bin/env python3
"""build_sika_atm_de_report.py — Sika Automotive DE (KNR 413276+493163), gewichtsbasiert"""
import math, re
...
BI_CACHE = Path('output/bi_cache_sika_atm_de.pkl')
OUT_XLSX = Path('output/billing_report/sika_atm_de_dinas_vergleich.xlsx')
OUT_XLSX.parent.mkdir(exist_ok=True)
KNRS = [413276, 493163]; KNR, KUNDE, BASIS = '413276+493163', 'Sika Automotive DE', 'EUR/100kg'
...
if BI_CACHE.exists():
    ...
    df_knr = df_all[df_all['Kunden Nr BK'].isin(KNRS)].copy()
    df_knr.to_pickle(BI_CACHE)
    print(f'KNR {KNRS}: {len(df_knr)} Zeilen, Cache gespeichert')
...
def build_main_sheet(ws):
    ws.title = 'Sika Automotive DE'
    ws.freeze_panes = 'A3'
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=N)
    wc(ws,1,1, f'Sika Automotive DE ({KNR}) — Dinas PRE vs AX POST  |  '
...
print(f'  Sheet "Sika Automotive DE": {last_row} Zeilen')
