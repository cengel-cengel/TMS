"""Fischerwerke (409480) Dinas-AX Vergleich Excel."""
import sys, pickle
...
OUT = Path("output/erka_lieferung/Fischerwerke_Vergleich.xlsx")
...
with open("output/bi_top20_data.pkl","rb") as f:
    bi = pickle.load(f)["df"]
dinas = pickle.load(open("output/dinas_cache_409480.pkl","rb"))
post  = bi[(bi["Kunden Nr BK"]==409480)&(bi["periode"]=="POST")].copy()
zgi   = load_zgi_map("Fischerwerke.xlsx")

n_clust, n_d, n_a = build_and_save(
    [409480], dinas, post, zgi, soll_fn,
    "Fischerwerke GmbH", OUT, "Fischerwerke.xlsx")
print(f"Fischerwerke: {n_clust} Cluster, {n_d} Dinas, {n_a} AX  → {OUT}")
