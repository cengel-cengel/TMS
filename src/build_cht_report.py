...
NK_XLSX     = Path('/home/user/TMS/data/extracted/v1/Noerpel AI/CHT/Nebenbedingungen DINAS/NK CHT.xlsx')
DINAS_DIR   = Path('/home/user/TMS/data/extracted/v1/Noerpel AI/CHT/Rechnungen/Rechnungen DINAS')
CACHE       = Path('/home/user/TMS/output/dinas_cache_486073.pkl')
REPORT_DIR  = Path('/home/user/TMS/output/billing_report')
OUT_XLSX    = REPORT_DIR / 'cht_dinas_vergleich.xlsx'
REPORT_DIR.mkdir(exist_ok=True)

KNR   = 486073
KUNDE = 'CHT Germany GmbH'
BASIS = 'EUR/100kg'
...
pre_lookup  = pre.apply(lambda r: _lookup_cht(r, tariff), axis=1)
post_lookup = post.apply(lambda r: _lookup_cht(r, tariff), axis=1)

pre['_soll_tarif']  = pre_lookup['soll_fracht']
pre['_zone']        = pre_lookup['zone_matched'].str.replace(r'^[A-Z]{2}\s+', '', regex=True)
pre['_basispreis']  = None  # CHT gibt keine base_rate zurück
