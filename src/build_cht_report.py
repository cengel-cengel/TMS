...
Datenquellen:
  - output/cht_dinas_vergleich.xlsx   (PRE/POST already matched)
  - cluster_vergleich/486073_CHT_Germany_GmbH_cluster.xlsx (Soll EUR per Auftrag)
"""
...
OUT_XLSX    = REPORT_DIR / 'cht_dinas_vergleich.xlsx'
...
COLS = [
    ...
    ('Sonstige EUR',  'Dinas Sonstige',        None,                8),
...
def ws_zusammenfassung(ws):
    ...
    rows = [
        ('CHT Germany GmbH (486073) -- Dinas PDF vs AX Vergleich', ''),
        ('', ''),
        ('-- PRE (Dinas-System) --', ''),
...
def ws_cluster(ws, top_n=20):
    ...
    mrow(ws,1,1,N,'CHT Germany GmbH (486073) -- Cluster-Vergleich: Unterfakturierung',
    ...
    for _,cl in stats.head(top_n).iterrows():
        ...
        row += 1
        mrow(ws,row,1,N,
             f'  Dinas Durchschnitt: {cl.avg_d_ges:,.2f} EUR  |  AX Durchschnitt: {cl.avg_ax_ges:,.2f} EUR  '
            ...
            if kpi is not None:
                ...
                for ci,(h,ps,_,_) in enumerate(COLS[1:],2):
                    ...
                    wc(ws,row,ci,v,fill('E2EFDA'),font(size=9),
                       'right' if e else 'left', EUR_FMT if e else None, BRD)
            if koi is not None:
                row += 1
