#!/usr/bin/env python3
"""
build_fischer_report.py
=======================
Erstellt output/billing_report/fischerwerke_dinas_vergleich.xlsx im GEZE-Format:
  Sheet 1 "Fischerwerke GmbH" — 23-Spalten Cluster-Vergleich (alt/neu, ▶-Header)
  Sheet 2 "NK_Konditionen"    — NK Fischerwerke.xlsx kopiert
"""
...
NK_XLSX     = Path('/home/user/TMS/data/extracted/v2/Fischer/Nebenbedingungen DINAS/NK Fischerwerke.xlsx')
DINAS_DIR   = Path('/home/user/TMS/data/extracted/v2/Fischer/Rechnungen/Rechnungen DINAS')
CACHE       = Path('/home/user/TMS/output/dinas_cache_409480.pkl')
REPORT_DIR  = Path('/home/user/TMS/output/billing_report')
OUT_XLSX    = REPORT_DIR / 'fischerwerke_dinas_vergleich.xlsx'
REPORT_DIR.mkdir(exist_ok=True)

KNR   = 409480
KUNDE = 'Fischerwerke GmbH & Co. KG'
BASIS = 'EUR/Sendung'
...
# ── Tarif laden ────────────────────────────────────────────────────────────
print('Lade Fischerwerke-Tarif...')
tariff = load_fischerwerke()
print(f'Tarif geladen: {len(tariff)} Einträge')
...
pre_lookup  = pre.apply(lambda r: _lookup_fischer(r, tariff), axis=1)
post_lookup = post.apply(lambda r: _lookup_fischer(r, tariff), axis=1)
...
def build_main_sheet(ws):
    ws.title = 'Fischerwerke GmbH'
    ws.freeze_panes = 'A3'

    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=N)
    wc(ws,1,1, f'Fischerwerke GmbH (409480) — Dinas PRE vs AX POST  |  '
...
print(f'  Sheet "Fischerwerke GmbH": {last_row} Zeilen')
print(f'  Sheet "NK_Konditionen":    NK Fischerwerke.xlsx kopiert')
